...
fn split_leaf<K: RedbKey + ?Sized>(
    ...
    for i in 0..division {
        ...
        builder.append::<K>(entry.key(), entry.value());
    }
    drop(builder);
...
fn repair_children<K: RedbKey + ?Sized>(
    ...
    if children.iter().all(|x| matches!(x, Subtree(_))) {
        ...
        let position = children
            .iter()
            .position(|x| matches!(x, PartialLeaf(_)))
            .unwrap();
        ...
        for i in 0..children.len() {
            match &children[i] {
                Subtree(handle) => {
                    if i == merge_with {
                        if let Some(PartialLeaf(partials)) = children.get(position) {
...
pub(in crate) fn make_mut_single_leaf<'a, K: RedbKey + ?Sized>(
    key: &[u8],
    value: &[u8],
    manager: &'a TransactionalMemory,
